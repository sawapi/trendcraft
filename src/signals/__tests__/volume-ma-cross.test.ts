...
describe("volumeMaCross", () => {
  ...
  it("should detect bearish cross when bullishOnly is false", () => {
    ...
    const volumes = [
      ...
      100, 120, 110, 130, 100, 90, 110, 120, 100, 95,
      105, 115, 100, 110, 120, 90, 100, 110, 105, 95,
  ...
  it("should respect minRatio parameter", () => {
    ...
    const volumes = [
      ...Array(20).fill(100),
      110, 120, 130, 140, 150, // slight increase
    ];
    const candles = createCandles(volumes);
  ...
  it("should throw error for invalid periods", () => {
    ...
    // Short period < 1
    expect(() =>
      volumeMaCross(candles, { shortPeriod: 0, longPeriod: 20 }),
    ).toThrow();

    // Long period <= short period
    expect(() =>
      volumeMaCross(candles, { shortPeriod: 20, longPeriod: 10 }),
    ).toThrow();
    expect(() =>
      volumeMaCross(candles, { shortPeriod: 20, longPeriod: 20 }),
    ).toThrow();
  });
  ...
  it("should calculate ratio correctly", () => {
    const volumes = [
      ...Array(20).fill(100),
      500, 500, 500, 500, 500, // 5x increase
    ];
    const candles = createCandles(volumes);
    const result = volumeMaCross(candles, {
      shortPeriod: 5,
  ...
  it("should track daysSinceCross", () => {
    const volumes = [
      ...Array(20).fill(100),
      300, 400, 500, 600, 700, 800, // sustained high volume
    ];
    const candles = createCandles(volumes);
    const result = volumeMaCross(candles, {
      shortPeriod: 5,
  ...
  it("should handle noisy data", () => {
    ...
    const volumes = [
      100, 90, 110, 95, 105, 100, 92, 108, 98, 103, // noisy baseline
      100, 95, 110, 105, 100, 98, 102, 100, 105, 95, // more baseline
      200, 180, 220, 250, 300, 280, 350, 400, // increasing with noise
    ];
    const candles = createCandles(volumes);
    const result = volumeMaCross(candles, {
      shortPeriod: 5,

...
- `@trendcraft/chart`: Bundle size limits enforced via `size-limit` + `@size-limit/esbuild` (brotli-compressed). Main ≤ 42.5 kB, Headless ≤ 11.9 kB, React/Vue wrapper ≤ 34 kB, Sparkline ≤ 5 kB (vanilla) / 7 kB (React/Vue), Replay ≤ 3 kB. Limits are ratcheted alongside intentional growth — `packages/chart/package.json` `size-limit` is the source of truth. Zero runtime dependencies; `trendcraft` (`>=0.3.0`), `react` (`>=18.0.0`), `vue` (`>=3.3.0`) are optional peer deps

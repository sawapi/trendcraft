...
3. **適用** — 安全レイヤーが変更を検証（日次±20%制限、パレット範囲、累積ドリフト上限）。新戦略にはWFA + モンテカルロゲートを適用
4. **追跡** — 過去の推奨結果を評価（改善/悪化/中立）し、次サイクルにフィードバック
...
2. 2日以上経過後、現在のスコアと適用前のスコアを比較
3. 判定: `improved`（+3ポイント以上）、`degraded`（-3ポイント以上）、`neutral`（それ以外）
4. 成果履歴は次回以降のレビューでLLMにフィードバック

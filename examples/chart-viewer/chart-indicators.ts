...
import type * as TrendCraft from "trendcraft";
import type {
  DivergenceSignal,
  VolumeAnomalyValue,
  VolumeProfileValue,
    ...
    series: [
      {
        ...
        type: "bar",
        data: histogram,
        itemStyle: { color: (params) => ((params.value as number) >= 0 ? "#26a69a" : "#ef5350") },
      },
      {
        name: "MACD",
        type: "line",
 ...
 */
export function updateStochChart(
  dates: string[],
  stochData: TrendCraft.Series<TrendCraft.StochasticsValue>,
  zoomStart: number,
): void {
 ...
 */
export function updateDmiChart(
  dates: string[],
  dmiData: TrendCraft.Series<TrendCraft.DmiValue>,
  zoomStart: number,
): void {
  if (!dmiChart) return;
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [{ yAxis: 25, label: { formatter: "25" } }],
        },
      },
      {
        name: "-DI",
 ...
 */
export function updateStochRsiChart(
  dates: string[],
  stochRsiData: TrendCraft.Series<TrendCraft.StochRsiValue>,
  zoomStart: number,
): void {
 ...
 * Update MFI chart
 */
export function updateMfiChart(
  dates: string[],
  mfiData: TrendCraft.Series<number | null>,
  zoomStart: number,
): void {
  if (!mfiChart) return;
 ...
 * Update CCI chart
 */
export function updateCciChart(
  dates: string[],
  cciData: TrendCraft.Series<number | null>,
  zoomStart: number,
): void {
  if (!cciChart) return;
 ...
 */
export function updateWillrChart(
  dates: string[],
  willrData: TrendCraft.Series<number | null>,
  zoomStart: number,
): void {
 ...
 * Update ROC chart
 */
export function updateRocChart(
  dates: string[],
  rocData: TrendCraft.Series<number | null>,
  zoomStart: number,
): void {
  if (!rocChart) return;
          ...
          data: [
            {
              ...
              label: { formatter: "70 Range", color: "#ffd93d" },
            },
            {
              yAxis: 85,
              lineStyle: { color: "#ef5350" },
              label: { formatter: "85 Tight", color: "#ef5350" },
            },
          ],
        },
      },
          ...
          data: [
            {
              ...
              label: { formatter: "ADX 20", color: "#4caf50" },
            },
            {
              yAxis: 25,
              lineStyle: { color: "#ff9800" },
              label: { formatter: "ADX 25", color: "#ff9800" },
            },
          ],
        },
      },
 ...
 * Update CMF chart
 */
export function updateCmfChart(
  dates: string[],
  cmfData: TrendCraft.Series<number | null>,
  zoomStart: number,
): void {
  if (!cmfChart) return;
    ...
    series: [
      {
        ...
        data: cmfData.map((d) => ({
          value: d.value,
          itemStyle: { color: d.value !== null && d.value >= 0 ? "#26a69a" : "#ef5350" },
        })),
        markLine: {
          silent: true,
          symbol: "none",
          ...
          data: [
            { yAxis: 0, label: { formatter: "0" } },
            {
              ...
              label: { formatter: "+0.1", color: "#26a69a" },
            },
            {
              yAxis: -0.1,
              lineStyle: { color: "#ef5350" },
              label: { formatter: "-0.1", color: "#ef5350" },
            },
          ],
        },
      },
          ...
          data: [
            {
              yAxis: 2.0,
              lineStyle: { color: "#ff9800" },
              label: { formatter: "2.0x High", color: "#ff9800" },
            },
            {
              yAxis: 3.0,
              lineStyle: { color: "#ef5350" },
              label: { formatter: "3.0x Extreme", color: "#ef5350" },
            },
          ],
        },
        markPoint: { data: markPointData },
  ...
  const option: echarts.EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        let status = "Neutral";
        if (data.isConfirmed)
          status = data.priceTrend === "up" ? "Confirmed Bullish" : "Confirmed Bearish";
        if (data.hasDivergence)
          status = data.priceTrend === "up" ? "Bearish Divergence" : "Bullish Divergence";
        return `${p.name}<br/>Price: ${pTrend}<br/>Volume: ${vTrend}<br/>Confidence: ${data.confidence.toFixed(0)}%<br/>Status: ${status}`;
      },
    },
    title: {
      text: "Volume Trend Confirmation",
      left: 10,
      top: 0,
      textStyle: { color: "#888", fontSize: 12, fontWeight: "normal" },
    },

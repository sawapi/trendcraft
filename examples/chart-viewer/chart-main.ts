...
import {
  createPerfectOrderMarkPointsEnhanced,
  createRangeBoundMarkAreas,
  createSqueezeMarkPoints,
  createSupportResistanceLines,
  updateCrossEventsList,
  updatePerfectOrderEventsListEnhanced,
  updateRangeBoundEventsList,
} from "./chart-signals";
import {
  currentCandles,
  currentZoomRange,
...
import { createLineSeries, formatDate } from "./utils";
...
export function updateChart(): void {
  ...
  if (indicators.bb) {
    ...
    series.push(
      createLineSeries(
        "BB Upper",
        bbData.map((d) => ({ time: d.time, value: d.value.upper })),
        "#6bcb77",
        "dashed",
      ),
    );
    series.push({
      ...createLineSeries(
        "BB Middle",
        bbData.map((d) => ({ time: d.time, value: d.value.middle })),
        "#6bcb77",
      ),
    ...
    series.push(
      createLineSeries(
        "BB Lower",
        bbData.map((d) => ({ time: d.time, value: d.value.lower })),
        "#6bcb77",
        "dashed",
      ),
    );
  ...
  if (indicators.donchian) {
    ...
    series.push(
      createLineSeries(
        "Donchian Upper",
        donchianData.map((d) => ({ time: d.time, value: d.value.upper })),
        "#1abc9c",
        "dashed",
      ),
    );
    series.push(
      createLineSeries(
        "Donchian Middle",
        donchianData.map((d) => ({ time: d.time, value: d.value.middle })),
        "#1abc9c",
      ),
    );
    series.push(
      createLineSeries(
        "Donchian Lower",
        donchianData.map((d) => ({ time: d.time, value: d.value.lower })),
        "#1abc9c",
        "dashed",
      ),
    );
  ...
  if (indicators.ichimoku) {
    ...
    series.push(
      createLineSeries(
        "Tenkan",
        ichimokuData.map((d) => ({ time: d.time, value: d.value.tenkan })),
        "#e74c3c",
      ),
    );
    series.push(
      createLineSeries(
        "Kijun",
        ichimokuData.map((d) => ({ time: d.time, value: d.value.kijun })),
        "#3498db",
      ),
    );
    series.push(
      createLineSeries(
        "Senkou A",
        ichimokuData.map((d) => ({ time: d.time, value: d.value.senkouA })),
        "#2ecc71",
        "dashed",
      ),
    );
    series.push(
      createLineSeries(
        "Senkou B",
        ichimokuData.map((d) => ({ time: d.time, value: d.value.senkouB })),
        "#e67e22",
        "dashed",
      ),
    );
    series.push(
      createLineSeries(
        "Chikou",
        ichimokuData.map((d) => ({ time: d.time, value: d.value.chikou })),
        "#9b59b6",
        "dotted",
      ),
    );
  ...
  if (indicators.keltner) {
    const kcData = TrendCraft.keltnerChannel(currentCandles, {
      emaPeriod: 20,
      atrPeriod: 10,
      multiplier: 2,
    });
    series.push(
      createLineSeries(
        "KC Upper",
        kcData.map((d) => ({ time: d.time, value: d.value.upper })),
        "#7c4dff",
        "dashed",
      ),
    );
    series.push(
      createLineSeries(
        "KC Middle",
        kcData.map((d) => ({ time: d.time, value: d.value.middle })),
        "#7c4dff",
      ),
    );
    series.push(
      createLineSeries(
        "KC Lower",
        kcData.map((d) => ({ time: d.time, value: d.value.lower })),
        "#7c4dff",
        "dashed",
      ),
    );
    ...
    dataZoom: [
      {
        type: "inside",
        xAxisIndex: [0, 1],
        start: Math.max(0, 100 - (150 / currentCandles.length) * 100),
        end: 100,
      },
      {
        type: "slider",
  ...
  if (indicators.stochrsi) {
    ...
    const stochRsiData = TrendCraft.stochRsi(currentCandles, {
      rsiPeriod: 14,
      stochPeriod: 14,
      kPeriod: 3,
      dPeriod: 3,
  ...
  requestAnimationFrame(() => {
    ...
    getSubCharts().forEach((chart) => {
      if (chart) {
        chart.setOption(
          { dataZoom: [{ start: currentZoomRange.start, end: currentZoomRange.end }] },
          { lazyUpdate: true },
        );
      }
    });

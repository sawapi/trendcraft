...
import type { SqueezeSignal, PerfectOrderValueEnhanced, RangeBoundValue, TrendReason } from 'trendcraft';
 ...
 */
export function updatePerfectOrderEventsListEnhanced(show: boolean, poData: TrendCraft.Series<PerfectOrderValueEnhanced>): void {
  ...
  poData.forEach((po) => {
    ...
    if (po.value.confirmationFormed && po.value.state === 'BULLISH_PO') {
      events.push({ time: po.time, type: 'bullish_confirmed', confidence: po.value.confidence, strength: po.value.strength, persistCount: po.value.persistCount });
    }
    if (po.value.confirmationFormed && po.value.state === 'BEARISH_PO') {
      events.push({ time: po.time, type: 'bearish_confirmed', confidence: po.value.confidence, strength: po.value.strength, persistCount: po.value.persistCount });
    }
    if (po.value.state === 'PRE_BULLISH_PO' && po.value.persistCount === 1 && !hasEverConfirmedBullish) {
      events.push({ time: po.time, type: 'pre_bullish', confidence: po.value.confidence, strength: po.value.strength, persistCount: po.value.persistCount });
    }
    if (po.value.state === 'PRE_BEARISH_PO' && po.value.persistCount === 1 && !hasEverConfirmedBearish) {
      events.push({ time: po.time, type: 'pre_bearish', confidence: po.value.confidence, strength: po.value.strength, persistCount: po.value.persistCount });
    }
    if (po.value.breakdownDetected) {
      events.push({ time: po.time, type: 'breakdown', confidence: po.value.confidence, strength: 0, persistCount: 0 });
    }
    if (po.value.collapseDetected) {
      events.push({ time: po.time, type: 'collapsed', confidence: po.value.confidence, strength: 0, persistCount: 0 });
    }
  });
  ...
  poData.forEach((po) => {
    ...
    if (po.value.pullbackBuySignal && po.value.type === 'bullish') {
      ...
      const gapPercent = shortMa !== null && midMa !== null && midMa !== 0
        ...
        : 0;
      events.push({ time: po.time, type: 'pullback_buy', confidence: po.value.confidence, strength: po.value.strength, persistCount: 0, gapPercent });
    }
  });
...
export function createSupportResistanceLines(
  ...
  lines.forEach(line => {
    ...
    const color = isResistance
      ? (line.isTight ? '#ff5252' : '#ef5350')
      : (line.isTight ? '#69f0ae' : '#4caf50');
    ...
    result.push([
      {
        ...
        label: { show: true, position, formatter: `${isResistance ? 'R' : 'S'}: ${line.price.toFixed(0)}`, color, fontSize: 10 },
      },
 ...
 */
export function updateRangeBoundEventsList(show: boolean, rbData: TrendCraft.Series<RangeBoundValue>): void {
  ...
  rbData.forEach((rb) => {
    ...
    if (rb.value.state === 'TRENDING' && rb.value.trendReason !== null) {
      events.push({ time: rb.time, type: 'trending', rangeScore: rb.value.rangeScore, trendReason: rb.value.trendReason });
    }
  });

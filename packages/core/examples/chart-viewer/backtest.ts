...
const entryConditions: Record<string, () => Condition> = {
  ...
  poRsi: () =>
    TrendCraft.and(TrendCraft.poPlusEntry({ periods: [5, 25, 75] }), TrendCraft.rsiBelow(60)),
  bbLower: () => TrendCraft.bollingerTouch("lower"),
  // Stochastics
  stoch20: () => TrendCraft.stochBelow(20),
  stochCrossUp: () => TrendCraft.stochCrossUp(),
  stochOversoldCross: () => TrendCraft.and(TrendCraft.stochBelow(30), TrendCraft.stochCrossUp()),
  // DMI/ADX
  dmiBullish: () => TrendCraft.dmiBullish(20),
  dmiBullishStrong: () => TrendCraft.and(TrendCraft.dmiBullish(25), TrendCraft.adxStrong(30)),
  // Volume confirmation combos
  gcVolume: () =>
    TrendCraft.and(TrendCraft.goldenCrossCondition(5, 25), TrendCraft.volumeAboveAvg(1.5)),
  macdRsi: () => TrendCraft.and(TrendCraft.macdCrossUp(), TrendCraft.rsiBelow(50)),
  ...
  rbVolAnomaly: () =>
    TrendCraft.and(TrendCraft.rangeBreakout(), TrendCraft.volumeAnomalyCondition(2.0, 20)),
  rbVolTrend: () => TrendCraft.and(TrendCraft.rangeBreakout(), TrendCraft.volumeConfirmsTrend()),
  rbPoVol: () =>
    TrendCraft.and(
  ...
  stochVolAbove15: () =>
    TrendCraft.and(TrendCraft.stochCrossUp(), TrendCraft.volumeRatioAbove(1.5, 20)),
  ...
  atrGc: () =>
    TrendCraft.and(TrendCraft.atrPercentAbove(2.3), TrendCraft.goldenCrossCondition(5, 25)),
};
...
const exitConditions: Record<string, () => Condition> = {
  ...
  stochOverboughtCross: () =>
    TrendCraft.and(TrendCraft.stochAbove(70), TrendCraft.stochCrossDown()),
  // DMI/ADX
  ...
  rbExitCombo: () => TrendCraft.or(TrendCraft.inRangeBound(), TrendCraft.rangeConfirmed()),
  poCollapseOrRange: () =>
    TrendCraft.or(
  ...
  macdVolDiv: () => TrendCraft.and(TrendCraft.macdCrossDown(), TrendCraft.volumeDivergence()),
};
 ...
 */
export function runBacktest(candles: NormalizedCandle[], mainChart: echarts.ECharts): void {
  if (candles.length === 0) {
  ...
  // Get parameters
  const capital = Number.parseFloat(capitalInput.value) || 1000000;
  const stopLoss = Number.parseFloat(stopLossInput.value) || undefined;
  const takeProfit = Number.parseFloat(takeProfitInput.value) || undefined;
  const trailingStop = Number.parseFloat(trailingStopInput.value) || undefined;
  ...
  const commissionRate = Number.parseFloat(commissionRateInput.value) || 0;
  ...
  if (filteredCandles.length < 50) {
    alert(
      `Not enough data after start date filter (${filteredCandles.length} candles). Need at least 50.`,
    );
    return;
  }
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [{ yAxis: capital, label: { formatter: "Initial" } }],
        },
      },
    ],
...
export function updateChartWithTradeMarkers(
  ...
  const candlestickSeries = series?.find((s) => s.name === "Candlestick") as
    | echarts.CandlestickSeriesOption
    | undefined;
  if (candlestickSeries) {
    ...
    candlestickSeries.markPoint = {
      data: [
        ...existingMarkPointData,
        ...markPointData,
      ] as echarts.MarkPointComponentOption["data"],
    };
...
export function clearTradeMarkers(mainChart: echarts.ECharts): void {
  ...
  if (candlestickSeries) {
    ...
    if (existingMarkPointData.length > 0) {
      candlestickSeries.markPoint = {
        data: existingMarkPointData as echarts.MarkPointComponentOption["data"],
      };
    } else {

...
export const indicatorPresets: Record<string, IndicatorPreset> = {
  ...
  weisWave: withCompute("weisWave", (c, p) =>
    weisWave(c, { method: p.method ?? "close", threshold: p.threshold ?? 0 }),
  ),
  anchoredVwap: withCompute("anchoredVwap", (c, p) =>
    anchoredVwap(c, { anchorTime: p.anchorTime ?? 0, bands: p.bands ?? 0 }),
  ),
  emv: withCompute("emv", (c, p) =>
    easeOfMovement(c, { period: p.period ?? 14, volumeDivisor: p.volumeDivisor ?? 10000 }),
  ),
  volumeTrend: withCompute("volumeTrend", (c, p) =>
    volumeTrend(c, {
